...
def calculate_bmi():
   kg = int(weight_tf.get()) #получаем значение веса которое ввёл пользователь и преобразуем в целое число
   m = int(height_tf.get())/100 #получаем значение роста и преобразуем в целое число
   bmi = kg/(m*m) #рассчет имт
   bmi = round(bmi, 1) #округление результата до одного знака после запятой
 
   if bmi < 18.5:
       messagebox.showinfo('Результат', f'ИМТ = {bmi} у вас недостаточный вес')
...
cal_btn = Button( #виджет для добавления кнопки
   frame,
   text='Рассчитать ИМТ',
